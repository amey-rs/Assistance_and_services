	...
	private void initialize() {
			...
			public void keyReleased(KeyEvent e) {
				...
				try {
		            ...
		            String id = txtid.getText();
		                ...
		                pst = con.prepareStatement("select product,srno,quality from form where id = ?");
		                pst.setString(1, id);
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String product,srno,quality,id;
				product = txtproduct.getText();
				srno = txtsrno.getText();
				quality = txtquality.getText();
				id  = txtid.getText();
				try {
				pst = con.prepareStatement("update form set product= ?,srno=?,quality=? where id =?");
				pst.setString(1, product);
				            pst.setString(2, srno);
				            pst.setString(3, quality);
				            pst.setString(4, id);
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				id  = txtid.getText();
				try {
				pst = con.prepareStatement("delete from form where id =?");
				            pst.setString(1, id);
				            pst.executeUpdate();
